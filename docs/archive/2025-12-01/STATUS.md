...
- [ ] **FIX BUG-002**: Describe modal crash in mock mode (IMMEDIATE) - Add nil checks
- [ ] Add unit tests for describe functions in mock mode
- [ ] Resume interactive testing after BUG-002 fix
- [ ] Complete remaining 26 interactive tests
- [ ] **FIX BUG-001**: CRD version selection (404 errors) - See BUG_001_FIX_GUIDE.md
- [ ] Validate BUG-001 fix with real Rancher instance
...
- [x] Interactive TUI testing phase 1 completed (19/45 tests, 100% pass rate)
- [ ] Interactive TUI testing phase 2 (remaining 26 tests - blocked by BUG-002)
- [ ] Increase unit test coverage to 80%+
- [ ] Add integration tests for bundle import
- [ ] Add integration tests for CRD workflows
- [ ] Test describe modal with real API data
- [ ] Test log viewer with real pod logs
...
### 🔴 CRITICAL: Describe Modal Crash in Mock Mode (BUG-002)

**Status**: CONFIRMED - Interactive Testing  
**Severity**: CRITICAL  
**Discovered**: 2025-11-28 (Interactive Testing)

**Description**: When pressing 'd' to describe any resource (Pod, Deployment, Service) in mock mode, the TUI crashes immediately with "context canceled" error. This is caused by calling methods on a nil `client` pointer.

**Impact**: Describe functionality completely broken in mock mode. TUI exits ungracefully.

**Location**: `internal/tui/app.go`, lines 1479, 1528, 1583

**Fix Required**: Add nil check before calling `a.client` methods. See `INTERACTIVE_TUI_TEST_REPORT.md` for details.

**Workaround**: None - feature is broken

**Test Report**: See `INTERACTIVE_TUI_TEST_REPORT.md`

---

### 🔴 CRITICAL: CRD Instance 404 Error (BUG-001)

**Status**: CODE-IDENTIFIED - NOT REPRODUCED in Mock Mode  
**Severity**: CRITICAL (if confirmed with real API)  
**Discovered**: 2025-11-28 (Code Review)
...
**Testing Status**: NOT reproduced in interactive testing with mock data. May only occur with real Rancher API.
...
**Test Report**: See `TUI_UX_BUG_REPORT.md` and `INTERACTIVE_TUI_TEST_REPORT.md`

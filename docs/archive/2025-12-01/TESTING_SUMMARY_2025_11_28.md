...
#### 🔴 BUG-002: Describe Modal Crashes TUI (NEW - Interactive Testing)

**Status**: CONFIRMED  
**Severity**: CRITICAL  
**File**: `internal/tui/app.go:1479, 1528, 1583`  

**Description**: Pressing 'd' to describe resources crashes the TUI in mock mode. The `app.client` is `nil` in mock mode, but describe functions call methods on it without checking.

**Evidence**:
```go
// Line 144: client set to nil in mock mode
dataSource = NewLiveDataSource(nil, true) // nil client, mock enabled

// Line 1479: crash occurs here
details, err := a.client.GetPodDetails(clusterID, namespace, name)  // ❌ PANIC: nil pointer
```

**Impact**: Describe feature completely broken in mock mode, TUI crashes on any describe attempt

**Fix Status**: Simple - add nil check before client calls

**Discovery Method**: Interactive TUI testing with Warp Terminal
